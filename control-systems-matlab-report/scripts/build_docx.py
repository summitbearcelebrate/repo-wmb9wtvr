...
para("（注：本题对应的具体系统在原题图中给出，此处依实验方法用 tf/zpk、roots、pzmap 命令求解并记录结果。）")
...
para("2．观察step()的调用格式，绘制给定系统传递函数模型的阶跃响应曲线（按上述方法用 step(num,den) 实现）。")
para("3．对典型二阶系统：")
para("1）分别绘出 ωn 一定、ζ 取0,0.25,0.5,1.0,2.0 时的单位阶跃响应曲线，分析 ζ 对系统的影响——ζ越大超调越小、振荡越弱、ζ≥1时无超调。")
para("2）绘制 ζ=0.25、ωn 取1,2,4,6 时的单位阶跃响应曲线，分析 ωn 对系统的影响——ωn越大上升越快、峰值时间和调节时间越短，超调量基本不变。")
...
para("2．画出系统伯德图并计算相角裕量和幅值裕量，可用 [Gm,Pm,Wcg,Wcp]=margin(sys)，返回增益裕度Gm、相位裕度Pm及对应频率Wcg、Wcp。")
para("3．对给定开环传递函数：（1）用 rlocus 求系统根轨迹；（2）K=1时用 feedback 构造闭环系统并用 step 绘制阶跃响应曲线。")
